...
class SAC(object):
    def __init__(self, input_dim, q_output_dim, action_space, args):
        ...
        self.critic = QNetwork(input_dim+1, 1, 256).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)

        self.critic_target = QNetwork(input_dim+1, 1, 256).to(self.device)
            ...
            if self.automatic_entropy_tuning is True:
                self.target_entropy = - \
                    torch.prod(torch.Tensor(
                        action_space).to(self.device)).item()
                self.log_alpha = torch.zeros(
                    1, requires_grad=True, device=self.device)
                self.alpha_optim = Adam([self.log_alpha], lr=args.lr)

            self.policy = GaussianPolicy(
                6, 10, args.hidden_size, action_space).to(self.device)
