...
class ClutteredSceneEnv(gym.Env):
    ...
    def reset(self, mode, num_cuboids=30):
        ...
        if mode==0:
            self.spawn_cluttered_cuboids(num_objects=30, max_layers=5)
        elif mode==1:
            self.spawn_pyr_cluttered_cuboids(num_objects=30, max_layers=5)
        elif mode==2:
            self.spawn_vstack_cluttered_cuboids(num_objects=30, height=10)
        else:
            self.spawn_stack_cluttered_cuboids(num_objects=30, rows=5, cols=6)
    ...
    def spawn_pyr_cluttered_cuboids(self, num_objects=30, max_layers=5):
        layer_height_increment = 0.1
        ...
        self.target = random.choice(self.object_ids)
        # print(len(self.object_ids))

    def spawn_vstack_cluttered_cuboids(self, num_objects=30, height=10):
        cube_size = 0.08
    ...
    def spawn_stack_cluttered_cuboids(self, num_objects=30, rows=5, cols=6):
        cube_size = 0.08
        base_spacing = cube_size
        z_base = self.table1_height + 0.1
    ...
    def compute_reward(self):
        ...
        for obj_id in self.object_ids:
            if obj_id in self.last_com and obj_id in self.curr_com:
                ...
                if displacement > 3e-2:  # 1mm threshold
                    num_disturbed += 1
        ...
        if self.steps>1:
            self.num_disturbed += num_disturbed
            self.disturbance += total_displacement
        print(self.num_disturbed)
    ...
    def step(self, action, max_settle_steps=1000, check_interval=1):
        ...
        # reward = self.compute_her_reward(action,self.target)
        _, object_features = compute_features(partial, total)
        global_features = _.get("total_clutter", None)
        state = global_features - object_features
        state = pad_state(state)
        num_remaining = len(list(partial.keys()))
        return state, reward, action==self.target or num_remaining ==1, num_remaining
