...
import numpy as np
import cv2 as cv2
from utils_env import compute_features_goal, pad_state
...
class ClutteredSceneEnv(gym.Env):
    ...
    def reset(self, num_cuboids=30):
        ...
        self.last_com = None
        self.curr_com = None
        while not self._has_settled():
            p.stepSimulation()
        partial, total = self._get_obs()
        _, object_features = compute_features_goal(partial, total, self.target)
        
        global_features = _.get("total_clutter", None)
        global_features = np.append(global_features, 0)
        state = global_features - object_features
        state = pad_state(state)
        return state 

    def spawn_cluttered_cuboids(self, num_objects, max_layers=10):
        max_layers = 10
        layer_height_increment = 0.1
        x_base, y_base = 0, 0  # Base position for the stack
    
        for layer in range(max_layers):
            # Generate dimensions for the cuboid
            length, width, height = np.random.uniform(0.05, 0.1, 3)
    
            # Base z position for the cuboid
            z_pos = self.table1_height + layer * layer_height_increment + height / 2
    
            # Introduce small random disturbances to position and rotation
            x_disturbance = np.random.uniform(-0.01, 0.01)
            y_disturbance = np.random.uniform(-0.01, 0.01)
            rotation_angle = np.random.uniform(-0.05, 0.05)  # Small tilt in radians
    
            # Create transformation matrix for rotation around the z-axis
            quat = p.getQuaternionFromEuler([0, 0, rotation_angle])
            ...
            body_id = p.createMultiBody(
                ...
                basePosition=[x_base + x_disturbance, y_base + y_disturbance, z_pos],
            ...
            # Allow the stack to settle after placing each cuboid
            for _ in range(50):
                p.stepSimulation()
    
        # Select a target object (e.g., topmost cuboid) and make it green
    ...
    def compute_reward2(self, action):
        ...
        if action == self.target and num_moved != 0:
            reward+= 5
    ...
    def compute_reward1(self,action):
        # Ensure the necessary variables are initialized
        if self.last_positions is None:
            self.last_positions = {obj_id: p.getBasePositionAndOrientation(obj_id)[0]
                                   for obj_id in self.object_ids}
        ...
        if action == self.target and reward != 0:
            reward-= 50
    ...
    def step(self, action, max_settle_steps=1000, check_interval=1):
        ...
        reward = self.compute_reward2(action)
        _, object_features = compute_features_goal(partial, total,self.target)
        global_features = _.get("total_clutter", None)
        global_features = np.append(global_features, 0)
        if np.any(np.isnan(global_features)):
            import pdb;pdb.set_trace()
        if np.any(np.isnan(object_features)):
            import pdb;pdb.set_trace()
        state= global_features - object_features
        ...
        if np.any(np.isnan(state)):
            import pdb;pdb.set_trace()
        return state, reward, action==self.target or len(self.object_ids)<5, len(self.object_ids)
...
if __name__ == "__main__":
    ...
    while not done:
        ...
        state, reward, done,_ = env.step(selected_object)
        import pdb;pdb.set_trace()
