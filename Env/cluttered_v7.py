...
class ClutteredSceneEnv(gym.Env):
    ...
    def spawn_cluttered_cuboids(self, num_objects, max_layers=3):
        ...
        # Select a target object and make it green
        self.target = random.choice(self.object_ids)
        p.changeVisualShape(self.target, -1, rgbaColor=[0, 1, 0, 1])  # RGBA for green color
    ...
    def compute_reward2(self,action):
        ...
        action_pos = np.array(p.getBasePositionAndOrientation(action)[0])
        target_pos = np.array(p.getBasePositionAndOrientation(self.target)[0])
        dist = np.linalg.norm(action_pos-target_pos)
        reward-= 5*dist
